...
let eval_let_syntax env v =
  ...
  let rec loop bindings v =
    ...
    | T.Cons (Cons (Symbol sym, syntax), rest) -> (
        ...
        match v with
        | T.Syntax v -> loop ((sym, T.Macro v) :: bindings) rest
        | _          -> T.raise_syntax_error ~args:[ v ] "Need syntax" )
    | T.Empty_list -> Ok bindings
    | _ -> T.raise_syntax_error ~args:[ v ] "Invalid syntax"
  in
  let* bindings = loop [] v in
  let keywords = List.map fst bindings |> List.sort_uniq Stdlib.compare in
  if List.length bindings <> List.length keywords then T.raise_syntax_error ~args:[ v ] "duplicate keywords"
  else (
