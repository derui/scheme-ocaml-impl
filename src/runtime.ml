...
module Make (Library_producer : Library_producer.S) (Feature_query : Feature_query.S) : S = struct
  ...
  let is_requirement_filled requirement =
    ...
    let rec match_requirement cond =
      ...
      | F.Feature_requirement.And (fst, rest) -> match_requirement fst && List.for_all match_requirement rest
      | F.Feature_requirement.Or (fst, rest) -> match_requirement fst || List.exists match_requirement rest
