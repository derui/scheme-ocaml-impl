...
let set_cdr_test =
  [
    Alcotest.test_case "should update cdr in place" `Quick (fun () ->
        ...
        let actual = P.List_op.set_cdr @@ T.(cons target @@ cons (Number "3") Empty_list) in
...
let append_test =
  [
    Alcotest.test_case "should return empty list if no any arguments" `Quick (fun () ->
        let actual = P.List_op.append @@ parse "(())" in
